...
void run_atocar(int argc, char **argv)
{
    ...
    float thresh = find_float_arg(argc, argv, "-thresh", .2);//默认为0.2,置信小于0.2的不展示boundingbox
    ...
    char *cfg = argv[3];//获得配置文件名
    char *weights = (argc > 4) ? argv[4] : 0;//获得weights文件名
    char *filename = (argc > 5) ? argv[5]: 0;//获得待测试文件名
