...
void parse_net_options(list *options, network *net)//ļ
{
	//ADD PARAMETER
...
network parse_network_cfg(char *filename)//ڽļ configeration file analysis
{
    list *sections = read_cfg(filename);//ļ
    ...
    parse_net_options(options, &net);//ãpolicy,learning rate and etc.
...
list *read_cfg(char *filename)
{
    ...
    char *line;
    int nu = 0;
    list *sections = make_list();//ʼ˫
    section *current = 0;
    while((line=fgetl(file)) != 0){//ȡһ
        ++ nu;
        strip(line);//ȥƱзͿո
        switch(line[0]){
            case '[':
